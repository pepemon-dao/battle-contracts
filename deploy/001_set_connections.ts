...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  async function sync_config(contract: string) {
    ...
    await execute(contract, {from: deployer, log: true, gasLimit: 10000000 }, "addAdmin", pepemonConfig.address);

    console.log(`Syncig ${contract} config...`)
    await execute(PEPEMON_CONFIG, { from: deployer, log: true, gasLimit: 10000000 }, "syncContractConfig", contract);
  }
  ...
  */
  ...
  //await execute(PEPEMON_BATTLE, {from: deployer, log: true, gasLimit: 10000000 }, "addAdmin", pepemonMatchmaker.address);

  console.log("Setting PepemonMatchmaker's Contract as a PepemonRewardPool admin...")
  await execute(PEPEMON_REWARDPOOL, {from: deployer, log: true, gasLimit: 10000000 }, "addAdmin", pepemonMatchmaker.address);

  console.log("Setting PepemonMatchmakerPve's Contract as a PepemonBattle admin...")
  await execute(PEPEMON_BATTLE, {from: deployer, log: true, gasLimit: 10000000 }, "addAdmin", pepemonMatchmakerPve.address);

  console.log("Setting PepemonMatchmakerPve's Contract as a PepemonRewardPool admin...")
  await execute(PEPEMON_REWARDPOOL, {from: deployer, log: true, gasLimit: 10000000 }, "addAdmin", pepemonMatchmakerPve.address);

  console.log("Allow PepemonMatchmakerPve's Contract to manage Decks of the deployer (needed to add a deck to the pve)")
  await execute(PEPEMON_DECK, { from: deployer, log: true, gasLimit: 10000000 }, 'setApprovalForAll', pepemonMatchmakerPve.address, true);
